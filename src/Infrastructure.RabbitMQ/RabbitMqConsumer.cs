...
public class RabbitMqConsumer<T>
{
    ...
    public void StartConsuming(string exchange, string queueName)
    {
        ...
        consumer.Received += async (_, @event) =>
        {
            ...
            using var activity = RabbitMqDiagnostics.ActivitySource.StartActivity(
                activityName, ActivityKind.Consumer, parentContext.ActivityContext);
            ...
            activity?.SetTag("message", message);  // DEMONSTRATION ONLY
    ...
    private static void SetActivityContext(
        Activity? activity, string eventName, string operation)
    {
    ...
    private IEnumerable<string> ExtractTraceContextFromBasicProperties(
        IBasicProperties props, string key)
    {
